...
- The complete prior art chain (6 DOIs including DMN v3.0)

---

## New: `CognitiveNodeAdapter` — JEPA + LLM into BatchNode

**File:** `core/adapter.py`

Bridges `AbstractCognitiveNode` to the Lár `BaseNode` interface, making any
cognitive node (JEPA, diffusion, SSM, GNN, or any future model type) directly
usable inside `BatchNode` for concurrent parallel execution.

Without the adapter, `AbstractCognitiveNode` and `BaseNode` are separate class
hierarchies. The adapter closes the gap:

```python
from core import CognitiveNodeAdapter

# Wrap any AbstractCognitiveNode subclass for use in the Lár graph
batch = BatchNode([
    CognitiveNodeAdapter(my_jepa, input_key="obs", output_key="jepa_pred"),
    LLMNode(model_name="gemini/gemini-2.0-flash", prompt_template="...", output_key="llm_resp"),
    CognitiveNodeAdapter(other_jepa, input_key="obs", output_key="jepa_pred_2"),
])
```

**Key behaviours:**
- Reads `input_key` from `GraphState` → `cognitive_node.encode()` → `forward()` → `decode()` → writes `output_key`
- For `AbstractManifold` subclasses, reads `action_key` from state and calls `predict_target(context, action_vector)`
- Writes `__signal_type_{output_key}` metadata to state so downstream `ContextBridge` implementations know the signal type
- Degrades gracefully on error: sets `last_error` in state without crashing the spine
- Thread-safe: relies on `BatchNode`'s `deepcopy` isolation; the wrapped node must be stateless at inference time
